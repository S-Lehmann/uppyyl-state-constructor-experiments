...
class Plots:
    ...
    def __init__(self):
        """Initializes Plots."""
        self.data_base_folder = RES_DIR.joinpath(f'logs/clock_state_construction/data_logs/')
        self.output_base_folder = RES_DIR.joinpath(f'logs/clock_state_construction/plots/')
    ...
    def create_plot_seq_csc_length_after_every_step(
        ...
        for model_name, measure_data in selected_measures.items():
            ...
            ref_plot, = plt.plot([0, x_vals[50]], [max_seq_constr_length, max_seq_constr_length], '--', linewidth=0.5)

            colors.append(ref_plot.get_color())

        # Plot Trivial approach results
        for i, (model_name, measure_data) in enumerate(selected_measures.items()):
            # Trivial
            y_vals = list(map(lambda step_measure: step_measure["seq_trivial_length_min_max_avg"][0],
                              measure_data["derived_measures"]))
            plt.plot(x_vals[0:50], y_vals[0:50], 'x', markersize=markersize+1,
                     markeredgewidth=0.7, color=colors[i], label=f'"{model_name}" - Trivial')

        # Plot Rinast approach results
        for i, (model_name, measure_data) in enumerate(selected_measures.items()):
            # Rinast
            y_vals = list(map(lambda step_measure: step_measure["seq_rinast_length_min_max_avg"][0],
                              measure_data["derived_measures"]))
            plt.plot(x_vals[0:50], y_vals[0:50], 'o', markersize=markersize+1, color=colors[i],
                     fillstyle='none', markeredgewidth=0.7, label=f'"{model_name}" - Rinast')

        # Plot OC approach results
        for i, (model_name, measure_data) in enumerate(selected_measures.items()):
            # OC
            ...
            plt.plot(x_vals[0:50], y_vals[0:50], '^', markersize=markersize+1, color=colors[i],
                     markeredgewidth=0.5, label=f'"{model_name}" - OC')

        plt.xlabel('simulated steps')
        plt.ylabel('sequence length $|S|$')
        plt.legend(bbox_to_anchor=(0, 1.0), loc='upper left', ncol=3, prop=font_p, columnspacing=0.5)
    ...
    def create_plot_gen_time_after_every_step(
        ...
        for model_name, measure_data in all_measures.items():
            ...
            y_vals = list(map(lambda step_measure: max(
                step_measure["seq_oc_o_dbm_c_fcs_gen_time_min_max_avg"][2],
                step_measure["seq_oc_o_dbm_c_mcs_gen_time_min_max_avg"][2],
                step_measure["seq_oc_o_dbm_c_rcs_gen_time_min_max_avg"][2],
                step_measure["seq_oc_o_seq_c_fcs_gen_time_min_max_avg"][2],
                step_measure["seq_oc_o_seq_c_mcs_gen_time_min_max_avg"][2],
                step_measure["seq_oc_o_seq_c_rcs_gen_time_min_max_avg"][2],
            ), measure_data["derived_measures"]))
            plt.plot(x_vals, y_vals, 'o', markersize=markersize, fillstyle='none',
                     markeredgewidth=0.7, label=f'"{model_name}"')
        ...
        plt.yticks(np.arange(0, 0.055, 0.01))
        # plt.yscale("log")
        plt.legend(bbox_to_anchor=(1, 1), loc='upper right', ncol=2, prop=font_p)
        plt.grid(which='both', alpha=0.5)
        # ax.set_xticks(range(0,x_vals[-1]))
        # ax.xaxis.set_major_locator(MaxNLocator(integer=True))
        # ax.yaxis.set_major_locator(MaxNLocator(integer=True))
    ...
    def create_plot_seq_approx_length_for_varying_seq_ref_length(
        ...
        plt.errorbar(x_vals - 3, mean_vals, [mean_vals - min_vals, max_vals - mean_vals], fmt='ok', lw=1,
                     markersize=markersize, marker="o", markerfacecolor="red", markeredgecolor="red",
                     ecolor="black", elinewidth=1, capsize=2, label="O(DBM)")
        ...
        plt.errorbar(x_vals, mean_vals, [mean_vals - min_vals, max_vals - mean_vals], fmt='ok', lw=1,
                     markersize=markersize, marker="s", markerfacecolor="blue", markeredgecolor="blue",
                     ecolor="black", elinewidth=1, capsize=2, label="O(SEQ)")

        plt.xlabel('orig. sequence length $|S|$')
        plt.ylabel('reduced approx. sequence length $|S_{approx}|$')
    ...
    def create_plot_seq_constr_length_for_varying_seq_ref_length(
        ...
        plt.errorbar(x_vals - 3, mean_vals, [mean_vals - min_vals, max_vals - mean_vals], fmt='ok', lw=1,
                     markersize=markersize, marker="o", markerfacecolor="red", markeredgecolor="red",
                     ecolor="black", elinewidth=1, capsize=2, label="C(FCS)")
        ...
        plt.errorbar(x_vals, mean_vals, [mean_vals - min_vals, max_vals - mean_vals], fmt='ok', lw=1,
                     markersize=markersize, marker="s", markerfacecolor="blue", markeredgecolor="blue",
                     ecolor="black", elinewidth=1, capsize=2, label="C(MCS)")
        ...
        mean_vals = np.asarray(list(
            map(lambda x: x["all_runs_measures"]["derived_measures"]["seq_constr_rcs_length_min_max_avg"][2],
                all_measures)))
        plt.errorbar(x_vals + 3, mean_vals, [mean_vals - min_vals, max_vals - mean_vals], fmt='ok', lw=1,
                     markersize=markersize, marker="^", markerfacecolor="green", markeredgecolor="green",
                     ecolor="black", elinewidth=1, capsize=2, label="C(RCS)")

        plt.xlabel('original sequence length $|S|$')
        plt.ylabel('reduced constr. sequence length $|S_{constr}|$')
    ...
    def create_plot_seq_approx_time_for_varying_seq_ref_length(
        ...
        mean_vals = np.asarray(list(
            map(lambda x:
                ...
                all_measures)))
        plt.plot(x_vals, mean_vals, 'o', markersize=markersize + 1, color="red",
                 markeredgewidth=0.7, label="O(DBM)")
        ...
        mean_vals = np.asarray(list(
            map(lambda x:
                ...
                all_measures)))
        plt.plot(x_vals, mean_vals, 's', markersize=markersize + 1, color="blue",
                 markeredgewidth=0.7, label="O(SEQ)")
    ...
    def create_plot_seq_constr_time_for_varying_seq_ref_length(
        ...
        mean_vals = np.asarray(list(
            map(lambda x:
                ...
                all_measures)))
        plt.plot(x_vals, mean_vals, 'o', markersize=markersize + 1, color="red",
                 markeredgewidth=0.7, label="C(FCS)")
        ...
        mean_vals = np.asarray(list(
            map(lambda x:
                ...
                all_measures)))
        plt.plot(x_vals, mean_vals, 's', markersize=markersize + 1, color="blue",
                 markeredgewidth=0.7, label="C(MCS)")
        ...
        mean_vals = np.asarray(list(
            map(lambda x:
                ...
                all_measures)))
        plt.plot(x_vals, mean_vals, '^', markersize=markersize + 1, color="green",
                 markeredgewidth=0.7, label="C(RCS)")
